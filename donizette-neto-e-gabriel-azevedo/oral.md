...
5. Apliquem o ciclo dado → informação → conhecimento → decisão à ferragista: dê um exemplo de cada etapa e mostre como uma consulta SQL liga o dado à informação.
   R: Na ferragista, o ciclo dado pode ser aplicado ao controle de estoque. O dado é o registro bruto armazenado no banco, como o código do produto, a quantidade em estoque e a quantidade vendida em determinado pedido. A informação surge quando uma consulta SQL organiza esses registros, por exemplo, somando a quantidade vendida por produto e comparando com o estoque atual. O conhecimento aparece quando se interpreta essa informação e se percebe que determinados produtos possuem risco de faltar no estoque. A decisão ocorre quando o gestor utiliza essa análise para, por exemplo, priorizar a reposição desses itens ou negociar com fornecedores. Nesse processo, a consulta SQL faz a ligação entre o dado bruto e a informação organizada, pois permite agrupar e filtrar registros operacionais em indicadores úteis para a gestão.
